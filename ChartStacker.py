...
def load_csv_data(filename):
    ...
    try:
        df = pandas.read_csv(filename, header=None, skip_blank_lines=True, delimiter=';', decimal=',', encoding='ANSI')
        if len(df.columns) > 2:
...
class NewGUI():
    def __init__(self):
        ...
        # bind actions
        self.root.bind('<Right>', self.replot_reduce_p)
        ...
        self.root.bind('<Alt-Right>', self.replot_reduce_n)
        self.root.bind('<Alt-Left>',  self.replot_expand_p)
        ...
        anzeige.add_checkbutton(label="X-Achse", onvalue=1, offvalue=0, variable=self.check_xaxis, command=self.replot)
        anzeige.add_checkbutton(label="Y-Achse", onvalue=1, offvalue=0, variable=self.check_yaxis, command=self.replot)
        anzeige.add_checkbutton(label="X-Achse bis heute", onvalue=1, offvalue=0, variable=self.check_today, command=self.replot)
        ...
        for opt in options_xlim:
            zeitraum_menu.add_radiobutton(label=opt, value=opt, variable=self.radio_xlim, command=self.replot)
    ...
    def replot_expand_p(self, *args):
        self.load_and_plot(True, 0, 0.3)

    def replot_expand_n(self, *args):
        self.load_and_plot(True, -0.3, 0)

    def replot_shift_p(self, *args):
        self.load_and_plot(True, 0.3, 0.3)
...
class Plotwindow():
    ...
    def stackplot(self, input_data, color_input, xlims, ylims, dx_left, dx_right):
        ...
        # labels and ticks
        float_year_axis_labels(self.ax, xlim, fontsize=self.fontsize)
