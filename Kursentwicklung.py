...
def load_files(path):
    files_this = glob.glob(path + "*.csv", recursive=False)
    files_this.sort()
    files_sub = glob.glob(path + "**/*.csv", recursive=False)
    files_sub.sort()
    files_all = ["Datei wählen ..."] + files_this + files_sub
...
def load_csv_comments(filename):
    cmt = [[],[],[],[]]
    try:
        data = np.genfromtxt(os.path.splitext(filename)[0] + "_Kommentare.csv", delimiter=';', usecols=(0,1,2,3), skip_header=1, dtype=str)
        c1 = data[:,0]
        c2 = data[:,1]
        c3 = data[:,2]
        c4 = data[:,3]
...
def set_labels_and_ticks(ax, xlim, fontsize, dropdown, x_axis_visible, y_axis_visible):
    ...
    xdiff = xlim[1] - xlim[0]
    if xdiff > 8:
        ax.set_xlabel("Jahr", fontsize = fontsize)
        ...
        if xdiff > 8/12:
            ...
            ax.set_xlabel("Tag", fontsize = fontsize)
            ax.minorticks_on()
            x_grid_minor_alpha = 0.3
            x_grid_major_alpha = 0.3
            xticks_pos = []
...
class NewGUI():
    def __init__(self):
        ...
        if os.path.isfile("Kursentwicklung.conf"): 
            with open("Kursentwicklung.conf", "r") as conf:
        ...
        load.add_command(label="Dateien aktualisieren", command=self.refresh)
        load.add_command(label="Grafik zurücksetzen", command=self.replot_reset_axes)
        load.add_command(label="Grafik aktualisieren", command=self.replot_leave_axes)
        menubar.add_cascade(label="Laden", menu=load)

        # anzeige
        anzeige = tk.Menu(menubar, tearoff=0)
        anzeige.add_checkbutton(label="X-Achse", onvalue=1, offvalue=0, variable=self.check_xaxis, command=self.replot_leave_axes)
        anzeige.add_checkbutton(label="Y-Achse", onvalue=1, offvalue=0, variable=self.check_yaxis, command=self.replot_leave_axes)
        ...
        anzeige.add_checkbutton(label="Horizontal bis zum nächsten Wert", onvalue=1, offvalue=0, variable=self.check_horizontal, command=self.replot_reset_axes)
        anzeige.add_checkbutton(label="Niveau halten nach letztem Punkt", onvalue=1, offvalue=0, variable=self.check_hold, command=self.replot_reset_axes)
        anzeige.add_checkbutton(label="Kommentare", onvalue=1, offvalue=0, variable=self.check_comments, command=self.replot_leave_axes)
        anzeige.add_checkbutton(label="Beträge", onvalue=1, offvalue=0, variable=self.check_betrag, command=self.replot_leave_axes)
        anzeige.add_checkbutton(label="Stil der Kommentare/Beträge nach Umsatz", onvalue=1, offvalue=0, variable=self.check_style, command=self.replot_leave_axes)
        menubar.add_cascade(label="Anzeige", menu=anzeige)

        # settings
        settings = tk.Menu(menubar, tearoff=0)
        settings.add_checkbutton(label="Daten an gleichen Tagen verteilen", onvalue=1, offvalue=0, variable=self.check_distribute, command=self.replot_reset_axes)
        settings.add_checkbutton(label="Stapeln", onvalue=1, offvalue=0, variable=self.check_stack, command=self.replot_reset_axes)
        settings.add_checkbutton(label="→ Übertrag von ausgeblendeten Mengen", onvalue=1, offvalue=0, variable=self.check_shift, command=self.replot_reset_axes)
        settings.add_checkbutton(label="Normieren (ändert Stapeln)", onvalue=1, offvalue=0, variable=self.check_norm, command=self.toggle_stacking)
        ...
        menubar.add_cascade(label="Zeitraum", menu=zeitraum)
        ...
        menubar.add_cascade(label="Skala", menu=skala)
    ...
    def remove(self, i):
        self.dropdown[i].set("Datei wählen ...")
        self.remove_x[i].place_forget()
        self.root.update()
        self.replot_reset_axes()
    ...
    def replot_reset_axes(self, *args):
        self.load_and_plot(1)

    def replot_leave_axes(self, *args):
        self.load_and_plot(0)

    def load_and_plot(self, reset_axes, *args):
        if reset_axes == 0:
        ...
        color_input = []
        for i in range(int(self.radio_number.get())):
            if not self.dropdown[i].get() == "Datei wählen ...":
        ...
        # plot
        if data_input:
            self.plot_window.stackplot(data_input, color_input, old_xlims, old_ylims)
        ...
        for i in range(int(self.radio_number.get())):
            if self.dropdown[i].get() == "Datei wählen ...":
                self.remove_x[i].place_forget()
            else:
    ...
    def on_close(self):
        with open("Kursentwicklung.conf", "w") as conf: 
...
class Plotwindow():
        ...
        for i in range(len(cmt_text)):
            ...
            and cmt_y[i] > self.ax.get_ylim()[0] and cmt_y[i] < self.ax.get_ylim()[1]:
                ...
                if self.root.check_comments.get() and self.root.check_betrag.get():
                    this_label = str(cmt_text[i]) + " (" + str(abs(cmt_diff[i])) + ")"
                elif self.root.check_comments.get():
                    this_label = str(cmt_text[i])
                else:
                    this_label = str(abs(cmt_diff[i]))
                if cmt_diff[i] > 0:
    ...
    def stackplot(self, alldata, color_input, xlims, ylims):
        ...
        if self.root.check_yaxis.get():
            try:
                ...
                if min(diff_jahreserster) < 3*1/365 and min(diff_monatserster) < 3*1/365:
                    ...
                    self.root.root.title("Kursentwicklung"
